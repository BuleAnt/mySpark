...
object TopNBasic {
	def main(args: Array[String]) {
		...
		val lines = sc.textFile("src/test/resources/topNBasic.txt")
		val pairs = lines.map(line => (line.split("\t")(1).toInt, line.split("\t")(0).toString))
		val sortedPaired = pairs.sortByKey(boolean2Boolean(false))
	...
	def topNline(): Unit = {
		val sc = new SparkContext(new SparkConf().setAppName("TopN").setMaster("local"))
		val lines = sc.textFile("src/test/resources/topN.txt")
