...
object DF2RDDJoin {
	...
	def main(args: Array[String]) {
		...
		val peopleScoreDF = sqlContext.read.format("json").load("src/main/resources/my_peoples.json")
		...
		val nameScoresRDD = scoresDF.rdd.map(row => (row.getAs("name").asInstanceOf[String], row.getAs("score").asInstanceOf[Long]))
		val nameAgeRDD = nameAgeDF.rdd.map(row => (row.getAs("name").asInstanceOf[String], row.getAs("age").asInstanceOf[Long]))
