package spark_mllib.book_huang.recommend
...
class RecommendedItem {
  ...
  def Recommend(items_similar: RDD[ItemSimi],
    ...
    //   1 矩阵计算——i行与j列join
    val rdd_app1_R1_pair = rdd_app1_R1.map(f => (f._1, (f._2, f._3)))
    val user_prefer1_pair =  user_prefer1.map(f => (f._2, (f._1, f._3)))

    //val rdd_app1_R2 = rdd_app1_R1.map(f => (f._1, (f._2, f._3))).join(user_prefer1.map(f => (f._2, (f._1, f._3))))
    val rdd_app1_R2 = rdd_app1_R1_pair.join(user_prefer1_pair)
    //   2 矩阵计算——i行与j列元素相乘
  ...
  def Recommend(items_similar: RDD[ItemSimi],
    ...
    val rdd_app1_R1_tuple =rdd_app1_R1.map(f => (f._1, (f._2, f._3)))
    val user_prefer1_tuple = user_prefer1.map(f => (f._2, (f._1, f._3)))
    val rdd_app1_R2 = rdd_app1_R1_tuple.join(user_prefer1_tuple)
    //val rdd_app1_R2 = rdd_app1_R1.map(f => (f._1, (f._2, f._3))).join(user_prefer1.map(f => (f._2, (f._1, f._3))))
    val a1 = rdd_app1_R1.map(f => (f._1, (f._2, f._3)))
    val a2 = user_prefer1.map(f => (f._2, (f._1, f._3)))
    //   2 矩阵计算——i行与j列元素相乘
    val rdd_app1_R3 = rdd_app1_R2.map(f => ((f._2._2._1, f._2._1._1), f._2._2._2 * f._2._1._2))
    //   3 矩阵计算——用户：元素累加求和
    val rdd_app1_R4 = rdd_app1_R3.reduceByKey((x, y) => x + y)
